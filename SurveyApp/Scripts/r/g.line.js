 ...
 ** 
 \*/
        ...
        var ydim;
        if(opts.ydimCustom){
            ydim = chartinst.snapEnds(0, opts.ydimCustom, valuesy[0].length - 1);            
        }
        else {
 ...
 \*/
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            ...
            for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                ...
                var jjjjj = paper[Raphael.is(sym, "array") ? sym[j] : sym](X, Y, (opts.width || 2) * 2);
                jjjjj.attr({ fill: colors[i], stroke: "none",opacity:0.7 });
                jjjjj.node.val = valuesy[i][j];
                jjjjj.node.labelxid = j;
                jjjjj.node.labelyid = (opts.labelyid!=null ? opts.labelyid : j);
                (Raphael.is(sym, "array") ? sym[j] : sym) && symset.push(jjjjj);
            ...
            for (var i = 0, ii = valuesy.length; i < ii; i++) {
                for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                    var X = x + gutter + ((valuesx[i] || valuesx[0])[j] - minx) * kx,
                        ...
                        Y = y + height - gutter - (valuesy[i][j] - miny) * ky;
                    f ? (C = {}) : cvrs.push(C = paper.circle(X, Y, Math.abs(nearX - X) / 2).attr({ stroke: "#000", fill: "#000", opacity: 0.0 }));
                    C.x = X;
                    C.y = Y;
                    C.value = valuesy[i][j];
